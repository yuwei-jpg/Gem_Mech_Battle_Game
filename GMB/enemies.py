...
class Robot(pygame.sprite.Sprite):
    def __init__(self, x, y, win):
        ...
        for i in range(1, 5):
            image = pygame.image.load(f'Assets/Enemy/Enemywalk{i}.png')
            left = right = pygame.transform.scale(image, (self.size, self.size))
            right = pygame.transform.flip(left, True, False)
            self.walk_right.append(right)
            self.walk_left.append(left)
        for i in range(1, 5):
            image = pygame.image.load(f'Assets/Enemy/Enemyhit{i}.png')
            image = pygame.transform.scale(image, (self.size, self.size))
            self.hit_list.append(image)
        for i in range(1, 9):
            image = pygame.image.load(f'Assets/Enemy/Enemydead{i}.png')
            image = pygame.transform.scale(image, (self.size, self.size))
